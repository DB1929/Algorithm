         ...
         %% Step size
               ...
               elseif Opt.N == 1
                    ...
                    if  (grad_final'*grad_final/nDim > 1E-5)
                        if TF.C2>0 
                            eta = Armijo(zKTInst,miniTLabel,[Syndata_P;Syndata_N],[ones(Syn_l_p,1);-1*ones(Syn_l_n,1)],w,[wp;bp],TF,grad_final,grad_final);
                        else
                            eta = Armijo(zKTInst,miniTLabel,0,0,w,[wp;bp],TF,grad_final,grad_final);
                        end
                        w = w - eta * grad_final;
