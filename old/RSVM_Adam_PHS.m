function [Model,Report] = RSVM_Adam_PHS(Model,TF,Opt,Set,Inst,Label)
%% ========================================================================
% JIAN-PING SYU
         ...
         %% Step size
               ...
               elseif Opt.N == 1
                    ...
                    if  (grad_final'*grad_final/nDim > 1E-5)
                        if TF.C2>0 
                            eta = Armijo(zKTInst,miniTLabel,[Syndata_P;Syndata_N],[ones(Syn_l_p,1);-1*ones(Syn_l_n,1)],w,[wp;bp],TF,direct,grad_final);
                        else
