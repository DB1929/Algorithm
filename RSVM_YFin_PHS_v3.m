function [Model,Report] = RSVM_YFin_PHS_v3(Model,TF,Opt,Set,Inst,Label)
%% ========================================================================
% JIAN-PING SYU
% YellowFin with (1)Proximal Model (2)Hypergradient
        ...
        %% Proximal model
         ...
         %% Gradient                        
               % Final gradient
                ...
                grad_w = (TF.C*w(1:end-1) - grad_prox_w - 2*TF.C1*zKTInst(Ih,:)'*(gradw_part) ) ;
                grad_b = (TF.C*w(end) - grad_prox_b - 2*TF.C1*sum(gradw_part)); 
...
function stepsize = Armijo(Tdata,Tlabel,w,wp,TF,direct,grad)
%% ========================================================================
% Armijo Stepsize
%
% Inputs
...
w2 = w - direct;
obj1 = objf(Tdata,Tlabel,w,wp,TF);
obj2 = objf(Tdata,Tlabel,w2,wp,TF);
   if (obj1 - obj2) <= 1E-8
...
function value = objf(Tdata,Tlabel,w,wp,TF)
%% ========================================================================
% Evaluate the objective function value
%==========================================================================
temp     = 1 - Tlabel.*(Tdata*w(1:end-1,1)+w(end));
v   = max(temp,0);
    value = 0.5*TF.C*(w'*w) + TF.C1*(v'*v)/length(v) - TF.C3*w'*wp;
end
