function [Model,Report] = RSVM_PSA_PHS_v2(Model,TF,Opt,Set,Inst,Label)
%% ========================================================================
% JIAN-PING SYU
% PSA with (1)Proximal Model (2)Hypergradient (3)Synthetic Data
...
eta     = Opt.eta;
% PSA initial
psa_m   = Opt.psa.kai*(Opt.psa.alpha + Opt.psa.beta)/(Opt.psa.alpha - Opt.psa.beta);
psa_n   = Opt.psa.kai*2*(1-Opt.psa.alpha)/(Opt.psa.alpha - Opt.psa.beta);
psa_t   = 0;
psa_eta = Opt.psa.eta*ones(rs1,1);
psa_w1  = w;
        ...
        if TF.C2>0
         ...
         %% PSA update                           
               % Final gradient
                ...
                direct = psa_eta.*grad_final; %PSA direction
         ...
         %% Step size
           ...
           %% PSA algorithm
           %count the psa update step
           psa_t = psa_t + 1;   
           %psa_t = psa_t + ceil(nIh/Opt.psa.b);
               if(mod(psa_t,2*Opt.psa.b)==0)
               % (w - w2) / (w2 -w1)
               % avoid 1 / 0
                   psa_div   = Opt.psa.kai*ones(rs1,1);
                   psa_ind   = (psa_w2-psa_w1)~=0;
                   psa_ind_z = (psa_w2-psa_w1)==0;
                   psa_div(psa_ind)  = (w(psa_ind)-psa_w2(psa_ind))./(psa_w2(psa_ind)-psa_w1(psa_ind));
                   psa_div(psa_ind_z)= sign(w(psa_ind_z)-psa_w2(psa_ind_z)).*psa_div(psa_ind_z);
                   % psa_u
                   psa_ind_ov= abs(psa_div)>Opt.psa.kai;
                   psa_div(psa_ind_ov) = Opt.psa.kai*sign(psa_div(psa_ind_ov)); 
                   % Update psa_eta
                   psa_eta = psa_eta.* ((psa_m+psa_div)/(psa_m+psa_n+Opt.psa.kai));
                   psa_w1 = w;        
